...
top_ten_excellent_analysis %>% 
  ...
  scale_fill_manual("legend", values = c("red","salmon","pink","steelblue1","steelblue4"), label = c("Excellent","Good","Average","Poor","Never Again"))+
  theme_classic()+
...
b<-pizza_barstool %>% 
  mutate(top_ten = ifelse(name %in% pizza_names, "yes", "no")) %>% 
  filter(review_stats_all_average_score!=0, review_stats_community_average_score!=0) %>% 
  ggplot(aes(x=review_stats_all_average_score, y = review_stats_community_average_score, label=name))+
  geom_point(aes(color=top_ten))+
  geom_smooth(method = "lm", se = FALSE)+
  theme_minimal()
